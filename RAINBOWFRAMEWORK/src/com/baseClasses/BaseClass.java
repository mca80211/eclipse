	...
	public void click(By locator) throws Exception {
		click(ThreadLocalWebdriver.getDriver().findElement(locator));
	}
	...
	*/
	...
	public  String getRandomDateOfBirth() throws Throwable{
	...
	public  String formatDate(String dateFormat) throws ParseException, Throwable{
	...
	public  void main(String[] args) throws ParseException, InterruptedException {
	...
	public void clickRadioButton(WebElement elem) throws AutomationException {
		try {
			elem.click();
		} catch (RuntimeException localRuntimeException) {
			System.out.println("Error in clicking RadioButton:" + elem + "Fail");
			// PDFResultReport.addStepDetails("Select Radio button", "Radio button shoud be available", "Error in clicking RadioButton: " + localRuntimeException.getMessage(), "FAIL","N");
			throw new AutomationException("Error in clicking RadioButton: " + localRuntimeException.getMessage());
		}
	}
	public static void clickImage(WebElement elem) throws AutomationException{
		try {
			elem.click();
		}
		catch (RuntimeException localRuntimeException)
		{
			System.out.println("Error in clickng Image:"+elem+"Fail");
			//     PDFResultReport.addStepDetails("Click Image", "Image should be available", "Error in clikcing on Image: " + localRuntimeException.getMessage(), "FAIL","N");
			throw new AutomationException("Error in clikcing on Image: " + localRuntimeException.getMessage());
		}
	}
	...
	public void afterSuite() throws Exception{
		try{
			...
			File local=pdfResultReport.suiteFolder;
			htmlrep.generateHtmlReport(TestName);
			//Sendmail.ComposeGmail("from email.id", "To email id",local);
	...
	{ 
		...
		//tcno=str; 
		//System.out.println("TEst cases No :"+no);
		Thread.sleep(1000);
	...
	public void afterClass() throws Exception
	{
	PDFResultReport.createTestRunReport(reportDetails.get("Test Script Name"),reportDetails.get("Test Script MyWorksshop Document ID"),reportDetails.get("Test Script Revision No"),reportDetails.get("Test Author Name"),reportDetails.get("Test Script Type"),reportDetails.get("Requirement Document ID of System"),reportDetails.get("Requirement ID"),true);
	PDFResultReport.addSummaryReport(TestName,desc.get(r));
	r++;
	PDFResultReport.captureTestCaseEndTime();
	PDFResultReport.generatePDFSummaryReport(TestName);
	PDFResultReport.writeChartToPDF(PDFResultReport.docWriter);
